...
- **共享动作 ID 库**：GUI「FSM/LMT 查询」窗口底部新增 —「导出实测ID」把实时捕获历史导出为 `fsm_db_submission.csv`；「导入CSV合并」把别人分享的 ID 合进本地 `fsm_db.csv`；「提交到共享库」导出+复制并打开提交页；「获取最新库」从仓库下载最新库。**有人测过一次，别人就不用再逐招试了。**
