...
### ✨ 新增：组合导出 / 导入（分享给其它玩家）
- 左侧「组合(当前激活)」旁多了 **「导出」「导入」**：
  - **导出**：把当前武器的当前组合（含「默认」组合）保存成一个可分享文件（自带说明头 + `[WeaponW:组合名]` 段 + 全部条目）；
  - **导入**：选别人分享的组合文件 → 按条目**去重合并**进当前配置（自动跳过已有条目），导入后记得点「保存」；
  - 导入复用「合并旧版ini」的同一套解析，旧配置也能这样导。

### 🐞 修复 / 调整：音效路径保存方式
- **选择音效不再复制一份进 `sounds\`**：
  - 文件本来就放在数据目录 `sounds\`（或旧位置 `plugins\sounds\`）→ 仍保存相对路径 `sounds/x`；
  - 在其它位置选的音效 → **只保存原路径（绝对路径）**，插件与 GUI 试听都直接读原文件，不再产生副本；
  - 仅当文件名含 `;` 或 `,`（会破坏 ini 的列表解析）时才自动复制进 `sounds\` 并提示——极少数情况。
- 插件端 `Sound=` 也**直接支持绝对路径和 UNC 路径**（`F:\...\x.wav`、`\\server\share\x.wav`）。
