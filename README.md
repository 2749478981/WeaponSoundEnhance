...
- **派生攻击触发音效**：玩家做组合/派生动作时，按配置匹配到对应动作，播放一条音效。
- **音效格式宽兼容**：**wav / mp3 / ogg / flac 直接支持**，不用转换。
  - wav 支持 8/16/24/32 位整型、32/64 位浮点、A-law/µ-law（别再为“听不到声音”去转比特率了）；
  - 解码统一到 16-bit PCM；常见采样率（44.1k / 48k 等）**原样播放、不重采样**，保住原始音质；只有特殊采样率才转 44.1k。
...
- **组合分享**：GUI 工具栏 **「导出组合 / 导入组合」** —— 导出会把组合用到的音效一起复制到导出目录的 `sounds\`（路径自动改写），发给别人「txt + sounds」整个文件夹即可；导入会自动把旁边的音效复制进本地 `sounds\` 并按条目去重合并。
- **游戏内热键**：开关 / 重载 / 音量 / 额外音效 / 切换组合。
- **游戏聊天框指令**：聊天框打 `/wse ...` 即可改配置。
- **最大音质**：解码保留原始采样率（44.1k/48k/96k 等原样输出，只做 16-bit 归一 + 抖动），不再把 48k 音效降成 44.1k。
...
3. 把你想要的音效（**wav / mp3 / ogg / flac 都行**，无需转换）放进 `WeaponSoundEnhance\sounds\`。
...
### 常见问题
- **选音效时会不会复制一份进 sounds\**：v2.13 起**不再复制**，只保存路径 —— 文件本来就在 `sounds\` 用相对路径；在别处选的用绝对路径，插件与试听直接读原文件。把配置发给别人时注意：**绝对路径的音效文件也要一起给对方**，否则对方会 `audio not found`。
- **默认组合的条目保存后跑到别的组合里 / 某把武器读不出默认组合**：v2.8 起已修（旧版保存时默认组合没写 `[Weapon<类型>]` 段头，条目会掉进上一个命名组合的段里；武器不对还会串到别的武器，例如长枪条目变成「武器6的组合 DIO」）。已经存坏的配置：用新版 GUI 打开（状态栏会提示“已修正 N 条”）再保存一次即恢复。手写 ini 时记住：默认组合的条目放 `[Weapon3]` 段，命名组合放 `[Weapon3:组合名]` 段，条目里的 `WeaponType` 要和段头武器一致。
- **切了组合没反应 / 改了配置游戏里没变化**：先看 GUI 状态栏的 **配置文件** 路径 —— 必须是游戏读取的那份 `<游戏目录>\nativePC\plugins\WeaponSoundEnhance\WeaponSoundEnhance.ini`。v2.7 起 GUI 默认就加载这一份，路径不一致时会红字提示并给「切换到游戏目录的 ini」按钮。另外配置里**一条条目都没有**时，游戏里自然什么都切不动。
- **Ctrl+F11 切组合没反应**：v2.7 起一定会有回显（组合名 / 只有 1 个组合 / 该武器没有条目 / 未进场景），并且切完会写回 ini 的 `[Active]`，重载和重开游戏都保持。热键不好用可以直接在聊天框打 `/wse combos` 看列表、`/wse combo 组合名` 切换。
...
- **升级后音效全都不响、日志里全是 `audio not found` / `audio decode failed`**：v2.3 起音效应放在 `plugins\WeaponSoundEnhance\sounds\`；还留在旧的 `plugins\sounds\` 也能读（v2.4 起自动回退，日志会提示建议搬移）。支持的格式为 wav/mp3/ogg/flac（其他格式如 AAC/m4a/wma 暂不支持）。音效是**启动时预载**的，放好文件后重进游戏或按 `Ctrl+F5` 重载。
